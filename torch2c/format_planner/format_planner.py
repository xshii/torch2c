...
from torch2c.format_planner._capabilities import (
...
def _get_non_absorbed_inputs(node, graph: Graph) -> list[str]:
    ...
    absorbed = set(node.absorbed_inputs.values()) if node.absorbed_inputs else set()
    return [tid for tid in node.inputs if tid not in absorbed]
