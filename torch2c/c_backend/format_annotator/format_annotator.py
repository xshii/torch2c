    ...
    for node in graph.nodes.values():
        ...
        for tid in node.outputs:
            ...
            if t is not None:
                ...
                t.dtype = dt
                updated_tensors += 1
                if old_fmt != fmt or old_dt != dt:
                    log_opt(
                        node, "format_annotator", "数据格式标注",
                        f"{tid}: {old_fmt}→{fmt}, {old_dt}→{dt}。"
                        f"NPU 各计算单元对输入格式有硬件约束"
                        f"（如 Cube src1 必须 NZ 分形格式），"
                        f"此处按{'算子标注' if (npu_out and (npu_out.format or npu_out.dtype)) else '全局目标'}确定目标格式",
                    )
