...
def _parse_pad_rules(config: dict) -> tuple[PadRule, dict[str, PadRule]]:
    ...
    bp = config.get("block_pad", {})
    default_raw = bp.get("default", {})
    ...
    for dtype, overrides in (bp.get("by_dtype") or {}).items():
...
def run(graph: Graph, config: dict) -> Graph:
    ...
    Args:
        ...
        config: 配置字典，可含 block_pad.default / block_pad.by_dtype。
...
def post_validate(graph: Graph) -> list[str]:
    ...
    使用硬编码兜底规则校验（post_validate 无 config 参数）。
    """
    fallback = PadRule(
    ...
    errors: list[str] = []
    for t in graph.tensors.values():
        errors.extend(_validate_shape(t.id, t.shape, fallback))
    return errors
