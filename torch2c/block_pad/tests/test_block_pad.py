...
from torch2c.block_pad.block_pad import (
...
class TestPadShape:
    ...
    def test_1d_tensor_aligns_to_single_dim(self):
        """1D tensor 对齐到 single_dim（默认 256）。"""
    ...
    def test_custom_single_dim(self):
        """自定义 single_dim 不同于 last_two_product。"""
...
class TestParseRules:
    ...
    def test_empty_config(self):
        ...
        assert default.single_dim == 256  # 默认等于 last_two_product
        assert by_dtype == {}

    def test_default_only(self):
        cfg = {"block_pad": {"default": {"last_dim": 32, "last_two_product": 512}}}
        default, _ = _parse_pad_rules(cfg)
        assert default.last_dim == 32
        assert default.last_two_product == 512
        assert default.single_dim == 512  # 未指定，跟随 last_two_product

    def test_explicit_single_dim(self):
        cfg = {"block_pad": {"default": {
    ...
    def test_by_dtype_overrides(self):
        ...
        assert by_dtype["int8"].single_dim == 1024  # 跟随 last_two_product
...
class TestRun:
    ...
    def test_per_dtype_1d(self):
        """不同 dtype 的 1D tensor 使用各自的 single_dim。"""
        cfg = {
            "block_pad": {
                "default": {"last_dim": 16, "last_two_product": 256},
                "by_dtype": {"int8": {"single_dim": 64}},
        ...
        run(g, cfg)
        assert g.tensors["t_fp16"].shape == [256]  # default single_dim=256
        assert g.tensors["t_int8"].shape == [64]    # int8 single_dim=64
