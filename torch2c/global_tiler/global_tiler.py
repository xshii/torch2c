...
def _estimate_compute_cycles(node: Node, graph: Graph, tile_size: int | None) -> float:
    ...
    if op in ("cube_matmul", "cube_matmul_bias") and len(node.inputs) >= 2:
        ...
        if a and b:
            ...
            return 2 * batch * m * n * k / 4096  # cube_ops_per_cycle
    ...
    for tid in node.outputs:
        ...
        if t:
            ...
            if tile_size and len(t.shape) >= 2:
                ...
                elem = int(elem * ratio)
            ops_per = 128 if cu == "vector" else 4096
            return max(1.0, elem / ops_per)
    return 1.0


def _estimate_dma_cycles(
    node: Node, graph: Graph, tile_size: int | None, cube_size: int,
) -> float:
    ...
    return max(1.0, total_bytes / 256)  # dma_bytes_per_cycle
...
def _evaluate_node(
    ...
    # no-tile baseline
    t_compute_full = _estimate_compute_cycles(node, graph, None)
    t_dma_full = _estimate_dma_cycles(node, graph, None, cube_size)
    ...
    for ts in candidates:
        ...
        for nb in [2, 1]:
            ...
            t_comp = _estimate_compute_cycles(node, graph, ts)
            t_dma = _estimate_dma_cycles(node, graph, ts, cube_size)
    ...
    for nid, node in graph.nodes.items():
        ...
        if tile_cfg:
            ...
            logger.info(
                ...
                nid, _get_tile_dim_size(node, graph) or 0,
