...
处理逻辑：
1. 按 npu_op 查裂解表 — 命中则 1:N 拆分为多个原子 NPU 算子
2. 未裂解的节点直接标记 is_mapped=True（确认为原生指令）
3. 对逐元素算子（vector_add 等）输入 shape 不匹配时插入 idma_broadcast
"""
...
def run(graph: Graph, config: dict) -> Graph:
    ...
    # 阶段 1：按 npu_op 查裂解表（不再用 is_mapped 门控）
    targets = [n for n in graph.nodes.values()
               if not n.is_mapped and n.npu_op and n.npu_op in rules]

    total_new_nodes = 0
    total_new_tensors = 0
    for node in targets:
        nn_count, nt_count = _decompose_node(graph, node, rules[node.npu_op])
        total_new_nodes += nn_count
        total_new_tensors += nt_count

    # 阶段 2：未裂解的节点标记 is_mapped=True（确认为原生指令）
    confirmed = 0
    for node in graph.nodes.values():
        if not node.is_mapped and node.npu_op:
            node.is_mapped = True
            confirmed += 1

    logger.info(
        "op_decomposition 完成: 裂解 %d 个算子→%d 节点, 确认 %d 个原生指令",
        len(targets), total_new_nodes, confirmed,
    )
...
def _decompose_node(graph: Graph, node: Node, rule: dict) -> tuple[int, int]:
    ...
    # 记录裂解决策
    step_desc = " → ".join(s["npu_op"] for s in steps)
    for nn in new_nodes:
        log_opt(nn, "op_decomposition", "算子裂解",
                f"NPU 无 {node.op_type} 原生指令，裂解为 {len(steps)} 个原子操作 "
                f"({step_desc})，每步可独立调度到对应计算单元")
...
def _insert_broadcast(graph: Graph, node: Node, input_idx: int) -> None:
    ...
    log_opt(bc_node, "op_decomposition", "广播展开",
            f"Vector 逐元素指令要求两个操作数 shape 完全一致，"
            f"但 {small_tid}{small_t.shape} ≠ {big_tid}{big_t.shape}，"
            f"通过 IDMA 将标量/小 tensor 复制扩展到 {bc_shape}")
