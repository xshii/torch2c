...
def test_basic_mapping():
    ...
    for node in graph.nodes.values():
        assert node.npu_op is not None, f"{node.id} ({node.op_type}) 缺少 npu_op"
        assert node.compute_unit is not None, f"{node.id} ({node.op_type}) 缺少 compute_unit"
        # op_mapping 不再设 is_mapped，由 op_decomposition 最终设置
        assert not node.is_mapped, "op_mapping 不应设 is_mapped"
...
def test_mixed_mapped_unmapped():
    ...
    n0 = g.get_node("n0")
    assert n0 is not None
    assert n0.npu_op == "cube_matmul"  # mm → cube_matmul
    n1 = g.get_node("n1")
    assert n1 is not None
    assert n1.npu_op is None  # unknown op 不在映射表
    n2 = g.get_node("n2")
    assert n2 is not None
    assert n2.npu_op == "vector_add"  # add → vector_add
