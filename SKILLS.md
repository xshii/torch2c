...
## Skill 8: 规格驱动测试 (`/spec-test`)

**触发词**: "规格测试"、"用例设计"、"spec test"、"测试设计"、"怎么写测试"、"决策表"、"不变量"

**能力要求**:
- 从规格/需求提取可测试属性（正向、反向、边界、极端）
- 用场景矩阵设计端到端用例（模型×内存×精度）
- 用决策表穷举 format×dtype 等输入组合（`@pytest.mark.parametrize`）
- 用不变量检查 Graph 引用完整性
- 每个 bug fix 附带回归测试

**5 种测试方法**:
1. **规格驱动** — 从 docs/ordr.md、CLAUDE.md、YAML 提取可测试属性
2. **用例驱动** — 场景矩阵（ST1-ST10 风格）
3. **不变量测试** — Graph 引用完整性、pass 后置条件
4. **决策表测试** — 参数化穷举所有输入组合
5. **回归测试** — bug fix 必附复现用例

**模板**: `templates/new_spec_test_template.py`

**参考文件**: `.claude/commands/spec-test.md`
